...
const PaymentForm = ({ amount, initialEmail, initialDetails, title, description, isSubscription, priceId }) => {
    const stripe = useStripe();
    const elements = useElements();
    const navigate = useNavigate();
    const [isLoading, setIsLoading] = useState(false);
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const endpoint = isSubscription
                ? 'https://shashikala-backend-gddy.onrender.com/create-subscription'
                : 'https://shashikala-backend-gddy.onrender.com/create-payment-intent';
            ...
            const confirmPayment = isSubscription
                ? stripe.confirmSetupIntent(clientSecret, {
                    payment_method: {
                        card: cardElement,
                        billing_details: { email, name: fullName },
                    },
                })
                : stripe.confirmCardPayment(clientSecret, {
                    payment_method: {
                        ...
                        billing_details: {
                            email,
                            name: fullName,
                            address: { line1: address1, line2: address2, city, state, postal_code: zipcode },
                        },
                    },
                });
...
const PaymentPage = () => {
    ...
    const zipcode = location.state?.zipcode || '';
    const cartItems = location.state?.cartItems || [];
    const isSubscription = !!location.state?.priceId;

    const title = location.state?.eventName
        ? `Register for ${location.state.eventName}`
        : isSubscription
            ? 'Subscribe to a Plan'
            : cartItems.length > 0
    ...
    const description = location.state?.eventName
        ? `Event Details: ${location.state.eventDate} | Venue: ${location.state.eventVenue}`
        : isSubscription
            ? `You're subscribing to the ${location.state.subscriptionName} plan.`
            : cartItems.length > 0
                ? `You're buying the following items: ${cartItems.map((item) => `${item.name} x ${item.quantity}`).join(', ')}`
